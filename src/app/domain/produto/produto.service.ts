...
interface SearchResult {
  ...
  function matches(produto: Produto, term: string, pipe: PipeTransform) {
    return produto.name.toLowerCase().includes(term.toLowerCase())
      || pipe.transform(produto.name).includes(term)
  }
  ...
  export class ProdutoService {
    ...
    private _countries$ = new BehaviorSubject<Produto[]>([]);
    ...
    constructor(private pipe: DecimalPipe) {
      this._search$.pipe(
        ...
        this._countries$.next(result.produtos);
    ...
    get produtos$() { return this._countries$.asObservable(); }
